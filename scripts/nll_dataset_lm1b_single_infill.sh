#!/bin/bash
#SBATCH -o logs/slurm/stories_infill_multi_nll.out          # output file (%j expands to jobID, %x expands to job_name) # TODO: For resuming, use %x only.
...
#experiment=lm1b_infill_multi_nll
experiment=stories_infill_single_nll

python -c 'import torch; print("num_gpus: ", torch.cuda.device_count())'
srun python -O -u src/pcdd/commands/add_nll_to_dataset.py \
