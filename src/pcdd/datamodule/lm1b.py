...
class LM1BDetokenizer:
    def __call__(
        self, examples: Mapping[Union[Literal["text"], str], List[str]]
    ) -> Mapping[Union[Literal["text"], str], List[str]]:
        return {"text": [detokenizer_lm1b_string(x) for x in examples["text"]]}
...
class LM1BDataModule(BaseDataModule):
    ...
    def __init__(
        ...
        super().__init__()
        # fixed parameters
        self.dataset_name = "lm1b"
        self.train_split = "train"
        self.val_split = "test"  # following MDLM paper
    ...
    def prepare_data(self) -> None:
        ...
        _cached = True
        for split in ["train", "test"]:  # lm1b has only two splits
...
class LM1BForIDLMDataModule(LM1BDataModule):
    def get_default_collator(
        self,
        tokenizer: Tokenizer,
        block_size: int,
...
class LM1BForILMStoppingDataModule(LM1BDataModule):
    def get_default_collator(
        self,
        tokenizer: Tokenizer,
        block_size: int,
...
class LM1BForMDLMDataModule(LM1BDataModule):
    def get_default_collator(
        self,
        tokenizer: Tokenizer,
        block_size: int,
...
class LM1BForARLMDataModule(LM1BDataModule):
...
class LM1BForILMStoppingInfillingDataModule(BaseDataModule):
    prediction_datasets = {
        "lm1b_single_20": "AvinashAmballa/LM1B_Masked-Single-Chunk-20",
        "lm1b_multi_20": "AvinashAmballa/LM1B_Masked-Chunks-20",
    }
    ...
    def __init__(
        ...
        noise_schedule: NoiseSchedule,
        *args,
        infill_dataset_name: Literal["lm1b"] = "lm1b",
        infill_percent: int = 20,
        infill_type: Literal["single", "multi"] = "single",
        num_dataset_workers: Optional[int] = None,
        predict_dataloader_kwargs: Optional[DataLoaderKwargs] = None,
        loss_on_padding: bool = True,  # true for mdlm
        block_size: int = 1024,
        **kwargs,
    ):
...
class LM1BForMDLMInfillingDataModule(LM1BForILMStoppingInfillingDataModule):
    def get_prediction_dataset(
        self,
        tokenizer: Tokenizer,
    ):
